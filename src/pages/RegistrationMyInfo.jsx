...
const RegistrationMyInfo = () => {
  ...
  useEffect(() => {
    participantApi.getProfile()
      .then((res) => {
        ...
        if (Array.isArray(p.religions) && p.religions.length > 0) setReligion(p.religions[0]);
  ...
  // Valider otherReligion uniquement quand "Other" est sélectionné
  useEffect(() => {
    if (religion === 'Other') {
  ...
  // Valider documentId quand isAuthor est 'yes'
  useEffect(() => {
    if (isAuthor === 'yes') {
      trigger('documentId');
    } else {
        ...
        >
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              {/* Other religion — visible uniquement si "Other" sélectionné */}
              {religion === 'Other' && (
                <div>
                  <textarea
